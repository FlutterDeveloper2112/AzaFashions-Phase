...
class MyAddressState extends State<BillingAddress> {
  FirebaseAnalytics analytics = FirebaseAnalytics();
  GlobalKey<ScaffoldState> billingscaffoldKey = new GlobalKey<ScaffoldState>();
  bool radioItem;
  ...
  void initState() {
    ...
    WebEngagePlugin.trackScreen("Billing Address Page");
    analytics.setCurrentScreen(screenName: "Billing Address");
  ...
  Widget build(BuildContext context) {
    connectionStatus = Platform.isIOS?connectivity.checkConnectivity1():Provider.of<ConnectivityStatus>(context);connectionStatus.toString()!="ConnectivityStatus.Offline"?  addressBloc.fetchAddressList("billing"):"";
    ...
    return WillPopScope(
      ...
      child: MediaQuery(
        ...
        child: Scaffold(
            ...
            key: billingscaffoldKey,
            appBar:AppBarWidget().myAppBar(context, "Billing Address",billingscaffoldKey),
            drawer: Drawer(
                child:SideNavigation(title: "Billing Address",)
            ),
            body:connectionStatus.toString()!="ConnectivityStatus.Offline"? RefreshIndicator(
                child: StreamBuilder(
                  ...
                  builder: (context, AsyncSnapshot<ProfileAddressList> snapshot) {
                    if (snapshot.hasData) {
                      if(snapshot.data.addressModel.length>0){
                                        ...
                                        InkWell(
                                          onTap: (){
                                            if(connectionStatus.toString()!="ConnectivityStatus.Offline"){
                                              checkoutBloc.fetchSelectAddress(snapshot.data.addressModel[index].addressId,"billing");
                                              ...
                                              UserTrackingDetails().checkoutShippingStarted(snapshot.data.addressModel[index].addressOne,"billing");
                                                      ...
                                                      children: <Widget>[
                                                        Container(
                                                          ...
                                                          child: Radio(
                                                              ...
                                                              value: snapshot.data.addressModel[index].selected==true && snapshot.data.addressModel[index].selection_type=="identical"||snapshot.data.addressModel[index].selection_type=="billing"?snapshot.data.addressModel[index].selected:false,
                                                              onChanged: (val) {
                                                                setState(() {
                                                ...
                                                Row(
                                                  ...
                                                  children: <Widget>[
                                                    Padding(
                                                        ...
                                                        child: InkWell(
                                                          onTap: (){
                                                            if(snapshot.data.addressModel[index].selected==false || snapshot.data.addressModel[index].selection_type=="shipping" ){
                                                              ...
                                                              ToastMsg().getLoginSuccess(context, "Select New Billing Address before removing the address.");
                                                            }
                                                    ...
                                                    Padding(
                                                        ...
                                                        child: InkWell(
                                                          onTap: () async{
                                                            ...
                                                            final result= await   Navigator.of(context).push(MaterialPageRoute(builder: (BuildContext context){return AddAddress("EDIT","Billing Address",snapshot.data.addressModel[index]);}));
                            ...
                            Container(
                              height: 43,
                                ...
                                child:  RaisedButton(
                                  color:  Colors.grey[300],
                                  onPressed: () async{
                                    final result= await   Navigator.of(context).push(MaterialPageRoute(builder: (BuildContext context){return AddAddress("ADD","Billing Address");}));
                                    result!=null?  await refreshData():null;
                                    setState((){
                                      ProfileUI.isEdited=false;
                                    });
  ...
  getSelectedRadio(List<AddressModel> addressModel){
    for(int index=0;index<addressModel.length;index++){
      if(addressModel[index].selected==true /*&& addressModel[index].selection_type=="identical"||addressModel[index].selection_type=="billing"*/){
  ...
  Future<void> refreshData() async{
    ...
    if(mounted){
      setState(() {
        addressBloc.fetchAddressList("billing");
      });
    }
